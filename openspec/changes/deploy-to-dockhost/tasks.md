...
- [x] 5.4 Задеплоить: запушить в `main` — дождаться, пока автодеплой Dockhost пересоберёт и
      передеплоит контейнер (без ручных команд на сервере — доступа к нему нет).
- [x] 5.5a Проверить, что данные на сетевом диске переживают именно пересборку образа
      (повторный `git push`), а не только restart того же контейнера: после ребилда `/start`
      от ранее зарегистрированного пользователя вернул ветку «уже существующий пользователь»
      (список команд), а не приветственное сообщение для нового — подтверждает, что запись в
      `users` не потерялась (см. `app/bot/handlers.py::start`, строки 323–373: ветвление по
      наличию `telegram_id` в БД).
- [ ] 5.5b Пройти остаток смоук-теста из задачи 4.3: `/latest`, `/summary` + кнопка «Полный
      текст» открывает WebApp по HTTPS, бот получает апдейты через `TELEGRAM_PROXY_URL`,
      `is_first_run` не рассылает старые законы всем подписчикам.
